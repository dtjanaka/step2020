...
public final class FindMeetingQueryTest {
  ...
  public void optionalAttendeeNoMatchingSlotsJustIgnore() {
    ...
    // person has split the restricted times and the optional attendee is not avaialable for any 
    // of these slots
    //
    // Events  :       |--A--|     |--B--|
    // Day     : |-----------------------------|
  ...
  public void optionalAttendeeSlotsTooSmallJustIgnore() {
    ...
    Collection<Event> events = Arrays.asList(
        new Event("Event 1", TimeRange.fromStartEnd(TimeRange.START_OF_DAY, TIME_0830AM, false),
            Arrays.asList(PERSON_A)),
        new Event("Event 1", TimeRange.fromStartEnd(TimeRange.fromStartDuration(TIME_0830AM, DURATION_15_MINUTES),
            Arrays.asList(PERSON_B)),
        new Event("Event 2", TimeRange.fromStartEnd(TIME_0900AM, TimeRange.END_OF_DAY, true),
            Arrays.asList(PERSON_A)));
